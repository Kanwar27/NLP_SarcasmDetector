...
#api = twitter.Api(consumer_key='tpAestpXtM2pYAlomfZr6LN7d', consumer_secret='MCQ1aVPypaBOZIlg7MDp36znULAIcmf9Cj8xfxodyVyLpILpQu', access_token_key='124163864-koQiHbqAF1QvLUzGqMb2ITvWk60jaa5yOsgJeaT7', access_token_secret='qdMSjnab0O49k1pnck0fgtVQre60VN7pb0qkSC2vSYwJE')
...
def createLimitedTrainingCorpus(corpusFile,tweetDataFile):
    ...
    for label in ["positive","negative"]:
        i=1
        for tweet in corpus:
            if tweet["label"]==label and i<=10:
                try:
                    #print(str(tweet["label"]) + " tweets Start")
                    status=api.GetStatus(tweet["tweet_id"])
                    #Returns a twitter.Status object 
                    print str(i) ,") Tweet fetched : " + status.text
                    tweet["text"]=status.text
                    #tweet is a dictionary which already has tweet_id and label (positive/negative/neutral)
                    # Add another attribute now, the tweet text 
                    trainingData.append(tweet)
                    i=i+1
                    #print(str(tweet["label"]) + " tweets End")
                except Exception, e:
                    print "e1 : " + str(e)
...
corpusFile=r'E:\NLP_SarcasmDetector\FinalProj2.7\Data\sanders-twitter-corpus.csv'
tweetDataFile=r'E:\\NLP_SarcasmDetector\\FinalProj2.7\\tweetDataFile.csv'
